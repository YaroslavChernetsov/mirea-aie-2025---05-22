...
def plot_categorical_bars(
    ...
    """
    Для категориальных/строковых колонок строит bar-plot с количеством значений.
    Сохраняет до `max_columns` графиков, каждый — для top-`top_k` категорий.
    Возвращает список путей к PNG.
    """
    ...
    for i, col in enumerate(cat_cols[:max_columns]):
        ...
        fig, ax = plt.subplots(figsize=(min(10, max(6, len(top_values) * 0.5)), 5))
        top_values.plot(kind="bar", ax=ax)
        ax.set_title(f"Top {len(top_values)} categories in '{col}'")
        ax.set_xlabel("Category")
        ax.set_ylabel("Count")
        ax.tick_params(axis="x", rotation=45)

        fig.tight_layout()
        out_path = out_dir / f"bar_{i+1}_{col}.png"
        fig.savefig(out_path)
        plt.close(fig)
        paths.append(out_path)
