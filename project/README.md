...
Единый отчёт по проекту формируется автоматически. Перед генерацией воспроизведите EDA и метрики:
...
`run_experiment.py` использует сохранённый snapshot production-модели. Для нового реального вызова OpenAI выполните `python scripts/run_experiment.py --live`; команда обновит snapshot и метрики. Результаты сохраняются в `reports/eda_summary.*`, `reports/experiment_predictions.json`, `reports/experiment_metrics.json` и `docs/project_report.md`.
...
EDA также оформлен как исполняемый ноутбук `notebooks/01_eda.ipynb`. Контрольный датасет находится в `data/experiment_dataset.json`; он содержит 15 размеченных вопросов, 7 тематических категорий и 5 типов задач без пропусков и дублей.
