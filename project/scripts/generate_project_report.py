...
EDA_OUTPUT = ROOT / "reports" / "eda_summary.json"
EXPERIMENT_OUTPUT = ROOT / "reports" / "experiment_metrics.json"
...
MODULE_ROLES = {
    ...
    "tests/test_experiment.py": "проверки датасета, EDA и расчёта экспериментальных метрик",
    "scripts/run_eda.py": "воспроизводимый EDA контрольного датасета",
    "scripts/run_experiment.py": "сравнение baseline и production OpenAI-пайплайна",
}
...
SELF_CHECKLIST = [
    ...
    [
        ...
        "Да",
        "`notebooks/01_eda.ipynb`, `scripts/run_eda.py`, `scripts/run_experiment.py`, `reports/eda_summary.json`, `reports/experiment_metrics.json`",
    ],
    [
        "4",
        "Есть baseline и улучшенная модель, сравнение по метрикам",
        "Да",
        "`scripts/run_experiment.py`, `reports/experiment_metrics.json`, разделы `21.2` и `21.3`",
    ],
    [
        "5",
...
def build_report() -> str:
    ...
    eda = json.loads(EDA_OUTPUT.read_text(encoding="utf-8"))
    experiment = json.loads(EXPERIMENT_OUTPUT.read_text(encoding="utf-8"))
    eda_rows = [[name, str(count)] for name, count in eda["categories"].items()]
    experiment_rows = [
        [
            "Baseline",
            "эвристики + универсальные шаблонные варианты",
            str(experiment["baseline"]["question_split_accuracy"]),
            str(experiment["baseline"]["type_accuracy"]),
            str(experiment["baseline"]["options_quality_rate"]),
            str(experiment["baseline"]["schema_valid_rate"]),
        ],
        [
            f"Improved ({experiment['model']})",
            "production prompt + JSON mode + Pydantic validation + retry",
            str(experiment["improved"]["question_split_accuracy"]),
            str(experiment["improved"]["type_accuracy"]),
            str(experiment["improved"]["options_quality_rate"]),
            str(experiment["improved"]["schema_valid_rate"]),
        ],
    ]
...
Таблица 1.3 - распределение {eda['questions_count']} вопросов по категориям:

{table(["Категория", "Количество"], eda_rows)}
...
Эксперимент воспроизводится командой `python scripts/run_experiment.py`. Сохранённый snapshot production-модели создан {experiment['snapshot_generated_at']} моделью `{experiment['model']}` на контрольном наборе из {experiment['baseline']['samples']} вопросов. Для обновления snapshot используется `python scripts/run_experiment.py --live` и `OPENAI_API_KEY`.
...
{table(["Вариант", "Описание", "question_split_accuracy", "type_accuracy", "options_quality_rate", "schema_valid_rate"], experiment_rows)}
