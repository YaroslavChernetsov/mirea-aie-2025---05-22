...
Дата формирования: 20.06.2026 01:03
...
| Метрика | Значение |
| --- | --- |
| Текстовых файлов учтено | 47 |
| Python-файлов | 31 |
| Всего строк в учтённых файлах | 3552 |
| Строк Python-кода | 2335 |
| API endpoints | 12 |
...
- reports/experiment_predictions.json
...
| app/services/openai_service.py | интеграция с OpenAI API, генерация вопросов и оценка ответов | 166 |
...
| scripts/generate_project_report.py | исходный файл проекта | 781 |
| scripts/run_eda.py | воспроизводимый EDA контрольного датасета | 82 |
| scripts/run_experiment.py | сравнение baseline и production OpenAI-пайплайна | 120 |
| tests/test_experiment.py | проверки датасета, EDA и расчёта экспериментальных метрик | 40 |
...
- `test_baseline_experiment_metrics_are_bounded`
- `test_eda_dataset_has_no_duplicates_or_missing_values`
- `test_saved_production_experiment_beats_baseline_options_quality`
- `test_liveness_ok`
...
| app/services/openai_service.py | 166 | 6635 |
...
| README.md | 270 | 9909 |
| reports/eda_summary.json | 29 | 542 |
| reports/eda_summary.md | 41 | 1331 |
| reports/experiment_metrics.json | 21 | 532 |
| reports/experiment_predictions.json | 156 | 6746 |
| requirements-dev.txt | 2 | 34 |
| requirements.txt | 10 | 194 |
| scripts/generate_project_report.py | 781 | 41898 |
| scripts/run_eda.py | 82 | 3249 |
| scripts/run_experiment.py | 120 | 4860 |
| tests/test_experiment.py | 40 | 1504 |
...
EDA выполнен скриптом `scripts/run_eda.py` и ноутбуком `notebooks/01_eda.ipynb` на размеченном наборе `data/experiment_dataset.json`. Машиночитаемый результат сохранён в `reports/eda_summary.json`, развёрнутый результат - в `reports/eda_summary.md`.

Таблица 1.3 - распределение 15 вопросов по категориям:
...
Эксперимент воспроизводится командой `python scripts/run_experiment.py`. Сохранённый snapshot production-модели создан 2026-06-19T21:59:48.191986+00:00 моделью `gpt-4o-mini` на контрольном наборе из 15 вопросов. Для обновления snapshot используется `python scripts/run_experiment.py --live` и `OPENAI_API_KEY`.

- `question_split_accuracy` - доля корректно выделенных вопросов;
- `type_accuracy` - доля корректно определённых типов вопросов;
- `options_quality_rate` - доля вопросов с четырьмя непустыми, уникальными и не универсально-шаблонными вариантами;
- `schema_valid_rate` - доля ответов, прошедших JSON/Pydantic validation.

Таблица 2.2 - результаты эксперимента:

| Вариант | Описание | question_split_accuracy | type_accuracy | options_quality_rate | schema_valid_rate |
| --- | --- | --- | --- | --- | --- |
| Baseline | эвристики + универсальные шаблонные варианты | 1.0 | 1.0 | 0.0 | 1.0 |
| Improved (gpt-4o-mini) | production prompt + JSON mode + Pydantic validation + retry | 1.0 | 1.0 | 1.0 | 1.0 |
...
- оба варианта выделили и классифицировали текущий контрольный набор, но только production pipeline сформировал содержательные варианты ответов;
- snapshot прошёл JSON/Pydantic validation полностью; validation и retry оставлены для защиты от будущих нестабильных ответов провайдера;
...
| 3 | Есть EDA и хотя бы один эксперимент с метриками | Да | `notebooks/01_eda.ipynb`, `scripts/run_eda.py`, `scripts/run_experiment.py`, `reports/eda_summary.json`, `reports/experiment_metrics.json` |
| 4 | Есть baseline и улучшенная модель, сравнение по метрикам | Да | `scripts/run_experiment.py`, `reports/experiment_metrics.json`, разделы `21.2` и `21.3` |
